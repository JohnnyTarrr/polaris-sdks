...
const POLARIS_API_URL = "https://api.thepolarisreport.com";
const CREDENTIALS_DIR = join(homedir(), ".polaris");
...
async function login(): Promise<void> {
  ...
  const authResp = await fetch(`${POLARIS_API_URL}/api/v1/auth/github/device`, {
    method: "POST",
  ...
  if (!authResp.ok) {
    ...
    console.error(`Polaris auth error: ${errData.message || "Authentication failed"}`);
    process.exit(1);
  }
  ...
  const keyResp = await fetch(`${POLARIS_API_URL}/api/v1/keys/create`, {
    method: "POST",
    headers: {
  ...
  console.log();
  console.log(`Authenticated as ${email} — API key saved to ~/.polaris/credentials`);
}

async function whoami(): Promise<void> {
  const apiKey = process.env.POLARIS_API_KEY || readCredentials();
  if (!apiKey) {
    console.error("Not logged in. Run `polaris login` to authenticate.");
    process.exit(1);
  }

  const resp = await fetch(`${POLARIS_API_URL}/api/v1/user/api-keys`, {
    headers: { Authorization: `Bearer ${apiKey}` },
  });

  if (resp.status === 401) {
    console.error("Invalid or expired API key. Run `polaris login` to re-authenticate.");
    process.exit(1);
  }
  if (!resp.ok) {
...
switch (command) {
  ...
  case "logout":
    ...
    break;
  default:
    console.log("Usage: polaris <command>");
    console.log();
