"""Polaris CLI — authenticate via GitHub and manage credentials."""
...
POLARIS_API_URL = "https://api.thepolarisreport.com"
CREDENTIALS_DIR = os.path.expanduser("~/.polaris")
CREDENTIALS_FILE = os.path.join(CREDENTIALS_DIR, "credentials")


def _read_credentials():
    """Read API key from ~/.polaris/credentials. Returns key string or None."""
    try:
...
def login():
    """Authenticate via GitHub device flow and save a Polaris API key."""
    ...
    # Step 3: Exchange GitHub token for Polaris JWT
    auth_resp = requests.post(
        "{}/api/v1/auth/github/device".format(POLARIS_API_URL),
        json={"access_token": gh_access_token},
    )
    if auth_resp.status_code != 200:
        msg = auth_resp.json().get("message", "Authentication failed")
        print("Polaris auth error: {}".format(msg), file=sys.stderr)
        sys.exit(1)
    ...
    key_resp = requests.post(
        "{}/api/v1/keys/create".format(POLARIS_API_URL),
    ...
    print()
    print("Authenticated as {} — API key saved to ~/.polaris/credentials".format(email))


def whoami():
    """Show the currently authenticated user."""
    api_key = os.environ.get("POLARIS_API_KEY") or _read_credentials()
    if not api_key:
        print("Not logged in. Run `polaris login` to authenticate.", file=sys.stderr)
        sys.exit(1)

    resp = requests.get(
        "{}/api/v1/user/api-keys".format(POLARIS_API_URL),
        headers={"Authorization": "Bearer {}".format(api_key)},
    )
    if resp.status_code == 401:
        print("Invalid or expired API key. Run `polaris login` to re-authenticate.", file=sys.stderr)
        sys.exit(1)
    if resp.status_code != 200:
...
def main():
    parser = argparse.ArgumentParser(prog="polaris", description="Polaris CLI")
